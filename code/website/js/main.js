...
/* If we are on a mobile device we must fix the splash-screen hight,
   otherwise the hight will jumo when the browser's navigation bar 
   is shown or hidden */
...
$('#login-button').click(function() {
  ...
  if(getSession() != null) {
    console.log('session is no NULL');
    leaveTo('profile.php');  
    return;
  }
...
$('#login-complete-button').click(function() {
  ...
  showLoadingOverlay(function() {
    ...
    $.rest.get('api/1.0/customer/login.php?', {user: user, pw: pw}, function(data) {
      ...
      if(!data.success) {
        showErrorOverlay('Login Error', '请输入正确的用户名和密码。');
      } 
    ...
    $.rest.put('api/1.0/customer/info.php', params, function(data) {
      ...
      if(!data.success) {
        ...
        /* Generic error  */
          ...
          showErrorOverlay('Registration Error', data.err_msg + ' [' + data.err_no + ']');
        }
      } 
...
function handleError(data) {
  ...
  if(!data.success) {
    ...
    var message = '无法从服务器得到返回值，请检查网络连接，咨询本站的程序猿。';
