<?php
  ...
  define("DB_USER", "elema_user");
  define("DB_PASS", "TKoUTQrk5BTVzqIb");
  define("DB_NAME", "elema");
  ...
  function check_customer_session($user, $session) {
    ...
    if(!$stmt = $db_link->prepare("SELECT COUNT(*) as ok FROM Customer WHERE nick=? AND session_id=?")) {
      db_error($answer, "Unauthorized1", ERROR_UNAUTHORIZED);
    }
    
    // Bind params
    if(!$stmt->bind_param("ss", $user, $session)) {
      db_error($answer, "Unauthorized2", ERROR_UNAUTHORIZED);
    }
    
    // Execute
    if(!$stmt->execute()) {
      db_error($answer, "Unauthorized3", ERROR_UNAUTHORIZED);
    }
    
    // Check result
/*    if($stmt->get_result()->fetch_assoc()['ok'] != 1) {
      db_error($answer, "Unauthorized4", ERROR_UNAUTHORIZED);
    }*/
  }
